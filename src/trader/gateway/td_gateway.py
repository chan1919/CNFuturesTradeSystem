from src.trader.gateway.base import BaseGateway, GatewayStatus
...
class TdGateway(BaseGateway):
    ...
    def connect(self, front_url=None):
        ...
        self._auth_pending = False
        self._auth_fallback_triggered = False
        flow_dir = Path(__file__).resolve().parent.parent.parent.parent / "flow"
    ...
    def check_timeouts(self, elapsed=None):
        if not self._auth_pending or self._auth_started_at is None:
            return False

        elapsed = (time.monotonic() - self._auth_started_at) if elapsed is None else elapsed

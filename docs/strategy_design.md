...
`IndicatorService` 基于 `BarCache` 管理每个合约的 K 线序列，并提供基础指标读取接口。当前支持纯函数 `sma()` / `ema()` / `macd()`，也支持通过 service 直接按合约读取。
...
class IndicatorService:
    ...
    def macd(self, instrument_id: str, fast_period: int = 12, slow_period: int = 26, signal_period: int = 9, field: str = "close_price") -> dict[str, float] | None: ...
```

指标服务同样不强行接入 `StrategyRuntime`。策略可以将完成的 `Bar` 放入自己的 `IndicatorService`，也可以直接调用纯函数处理自定义序列。

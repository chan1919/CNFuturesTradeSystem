...
if not positions:
    ...
    for p in positions:
        ...
        if direction == "2":  # long position, sell to close
            close_dir = "sell"
        ...
        total_vol = yd + today
        # DCE: use "close" for yd, "close_today" for today
        # But some exchanges don't allow close_today in day session
        # Try "close" first for the total, then split if needed
